...
export default function CoverLetterPage() {
  ...
  const handleCoverLetterGeneration = async () => {
    ...
    try {
      ...
      // Update resume analysis with missing skills if provided
      if (coverLetter.missing_skills && coverLetter.missing_skills.length > 0) {
        setResumeAnalysis(prev => prev ? {
          ...prev,
          missing_skills: coverLetter.missing_skills
        } : null);
      }
            ...
            {resumeAnalysis ? (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {/* Missing Skills Section - Resource Hub Link */}
                  {resumeAnalysis.missing_skills && resumeAnalysis.missing_skills.length > 0 && (
                    <div className="mt-4 p-4 rounded-xl bg-orange-50 border border-orange-100 dark:bg-orange-900/10 dark:border-orange-900/30">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <h4 className="flex items-center gap-2 text-sm font-semibold text-orange-800 dark:text-orange-200 mb-1">
                          ...
                          <p className="text-xs text-orange-700 dark:text-orange-300 mb-3">
                            We identified {resumeAnalysis.missing_skills.length} skills that could strengthen your profile for this role.
                          </p>
                          <div className="flex flex-wrap gap-1.5 mb-3">
                            {resumeAnalysis.missing_skills.map((skill) => (
                              <span key={skill} className="px-2 py-0.5 rounded text-xs font-medium bg-white/50 text-orange-700 dark:bg-black/20 dark:text-orange-300 border border-orange-200 dark:border-orange-800">
                                {skill}
                              </span>
                            ))}
                          </div>
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (resumeAnalysis.missing_skills) {
                              params.set('skills', resumeAnalysis.missing_skills.join(','));
                            }
                            window.open(`/resources?${params.toString()}`, '_blank');
                          }}
                          className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-lg bg-orange-100 hover:bg-orange-200 text-xs font-semibold text-orange-800 dark:bg-orange-900/40 dark:hover:bg-orange-900/60 dark:text-orange-100 transition-colors"
                        >
                          View Resources
